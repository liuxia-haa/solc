 ...
 */
    ...
    public:
        SourceUnit(
                int64_t _id,//节点在一个ast中的id
                SourceLocation const& _location,//在源代码中的位置
                std::optional<std::string> _licenseString,
                std::vector<ASTPointer<ASTNode>> _nodes //它的子节点们
        ):
                ASTNode(_id, _location), m_licenseString(std::move(_licenseString)), m_nodes(std::move(_nodes)) {}//初始化的过程
