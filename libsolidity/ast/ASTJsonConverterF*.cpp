...
namespace solidity::frontend
{
    ...
    void ASTJsonConverter::setJsonNode(//设置jsonNode节点，节点，名字，
            ASTNode const& _node,
            string const& _nodeName,
            initializer_list<pair<string, Json::Value>>&& _attributes//存储键值对
    )
    {
    ...
    void ASTJsonConverter::setJsonNode(//设置jsonNode节点，节点，节点类型，
            ASTNode const& _node,
    ...
    )
    {
        ...
        m_currentValue["id"] = nodeId(_node);//节点id
        m_currentValue["src"] = sourceLocationToString(_node.location());//定位源代码的位置信息
        ...
        for (auto& e: _attributes)
            m_currentValue[e.first] = std::move(e.second);//都是键值对
    }
...
//    }
    Json::Value ASTJsonConverter::typePointerToJson(Type const* _tp, bool _short)
    {
        Json::Value typeDescriptions(Json::objectValue);
        //为什么是bool类型，short
        typeDescriptions["typeString"] = _tp ? Json::Value(_tp->toString(_short)) : Json::nullValue;//类型标识符，_short:t_uint256 直接修改为int
        typeDescriptions["typeIdentifier"] = _tp ? Json::Value(_tp->identifier()) : Json::nullValue;
        return typeDescriptions;
...
//                make_pair("license", _node.licenseString() ? Json::Value(*_node.licenseString()) : Json::nullValue),
    ...
    {
        std::vector<pair<string, Json::Value>> attributes = {
                make_pair("name------->module", _node.name()),//该name 可以直接转换为module Test
                make_pair("F*-------------------contract->module", "module "+_node.name()),
    ...
    {
        std::vector<pair<string, Json::Value>> attributes = {
                make_pair("name------------------------------------------------------>let", _node.name()),//转换为let mathadd
...
//                make_pair("scope", idOrNull(_node.scope()))
    ...
    {
        std::vector<pair<string, Json::Value>> attributes = {
                make_pair("name----------->variabledeclaration:x y and return value", _node.name()),
                make_pair("F*--------------------------------------->", "("+_node.name()+":"),
                make_pair("F*--------------------------------------->typeName", toJson(_node.typeName())),
//                make_pair("nameLocation", sourceLocationToString(_node.nameLocation())),
...
//                make_pair("typeDescriptions", typePointerToJson(_node.annotation().type, true))
    ...
    bool ASTJsonConverter::visit(ElementaryTypeName const& _node)
    {
        std::vector<pair<string, Json::Value>> attributes = {
                ...
                make_pair("F*---------------------------typename",_node.typeName().toString()),
//                make_pair("typeDescriptions", typePointerToJson(_node.annotation().type, true))
        };
    ...
    {
        setJsonNode(_node, "Return------->functionStatement:return ", {
    ...
    bool ASTJsonConverter::visit(BinaryOperation const& _node)
    {
        std::vector<pair<string, Json::Value>> attributes = {
                make_pair("operator------->/", TokenTraits::toString(_node.getOperator())),
                make_pair("F*------------------->/operator",TokenTraits::toString(_node.getOperator())),
                make_pair("leftExpression-------->x", toJson(_node.leftExpression())),
                make_pair("rightExpression---------->y", toJson(_node.rightExpression())),
//                make_pair("commonType", typePointerToJson(_node.annotation().commonType)),
    ...
    bool ASTJsonConverter::visit(Identifier const& _node)
    {
        ...
        setJsonNode(_node, "Identifier", {
                make_pair("name", _node.name()),
                make_pair("F*---------------------------->identifier",_node.name()),
//                make_pair("referencedDeclaration", idOrNull(_node.annotation().referencedDeclaration)),
